...
<div class="py-10">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-3"> 
        @foreach ($chirps as $chirp)
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg flex space-x-2">
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-center">
                        <div>
                            <span class="text-gray-200">{{ $chirp->user->name }}</span>
                            <small class="ml-2 text-sm text-gray-200">{{ $chirp->created_at->format('j M Y, g:i a') }}</small>
                            @unless ($chirp->created_at->eq($chirp->updated_at))
                                <small class="text-sm text-gray-200"> &middot; {{ __('edited') }}</small>
                            @endunless
                        </div>
